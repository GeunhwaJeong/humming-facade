...
console.log('1) 로그인 (dave — 이 체인에서 bob/wasabi 미구독 팬)')
...
await page.fill(tid('loginUsernameInput'), 'dave.hum.haneul')
...
const carolPostVisible = await page.locator('text=carol의 구독자 전용 정원').count()
...
await page.waitForSelector('text=carol의 구독자 전용 정원', { timeout: 60000 })
...
await page.waitForSelector('text=carol의 구독자 전용 정원', { timeout: 30000 })
console.log('   ✅ 프로필 잠금 해제')
await page.screenshot({ path: SHOT('6-unlocked-profile') })

console.log('7) 게시물 상세: 팁 보내기 라벨')
await page.locator('text=carol의 구독자 전용 정원').first().click()
