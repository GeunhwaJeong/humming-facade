...
Gap backfill on rate-limited full nodes: the checkpoint tailing catches up over gRPC after a restart, one `GetCheckpoint` per missed checkpoint. On a public full node with a request limit that is fine for a deploy restart (tens of checkpoints) but not for hours of downtime. When `HUMMING_GRAPHQL_URL` is set (defaulted for the `sui-*` network map entries), a gap of at least `HUMMING_GAP_BACKFILL_MIN` checkpoints (default 300) is instead recovered with a handful of GraphQL event queries filtered to the package's modules, and the cursor jumps to the tip; the gRPC stream takes over from there. Events arrive in the same shape as from the stream, so the journal and its dedupe keys are unaffected. Networks served by your own full node leave the variable unset and keep the pure gRPC path; set it to an empty string to disable a default. Rate-limit responses during gRPC catch-up are retried with backoff instead of failing the boot.
