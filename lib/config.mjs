...
// 결제 통화는 메인넷과 같은 형상으로 Circle 테스트넷 USDC (developers.circle.com 게시 주소,
// faucet.circle.com에서 수령). 2026-08-28의 SUI 결제 티어(sora)는 이 형상과 타입이 달라 제외.
const SUI_TESTNET = {
...
// Sui 메인넷 배포 (2026-09-03 publish C7jTGgV4.., 체인 35834a8a). FEED/RULES는 부트스트랩이 채운다.
// 결제 통화는 네이티브 USDC($ 표시 확정에 따름)
const SUI_MAINNET = {
  PKG: '0xab4b13423a5f6d06d0c674d34cffcc3ef608b3ecf8995c046ddd08c5f1e02cec',
  FEED: '0xd0f89f42e610e92085e49798e77c61415576562e2ddac3c498746ffa812d2a71',
  RULES: '0xa115c8a771b498221d7f37e56e6fda420850ab9159cfbeed3dcf9782370d23df',
  FEE_CONFIG: '0xfdc95259beccc716d344a79be87681c2c65f102d7630e0d92ec118c418e2fe22',
  PREFS_REGISTRY: '0xe3c914089b09a0cae4fb0142e3a76244d492cf6c779fd2ccb09c10fdf4e069dd',
  NS_PKG: null,
  NS_SUB_PKG: null,
  NS_OBJ: null,
  HUM_PARENT_NFT: null,
  APP_WALLET: '0x2637c976c8d05e40217b937ee165bd5281418a82814cf4c5249320d5dca0bea0',
  DENY_RESERVED_TABLE: null,
  DENY_BLOCKED_TABLE: null,
...
// 갭 백필용 GraphQL (선택). 장시간 다운 뒤 놓친 구간을 체크포인트 단위 gRPC 읽기(체크포인트
// 수만큼의 호출) 대신 이벤트 필터 쿼리 몇 건으로 복원한다 — 요청 제한이 있는 공공 풀노드에서
// 재시작 복원이 제한에 막히지 않게 하는 장치. 자체 풀노드를 쓰는 네트워크(제한 없음)는
// 비워 두면 기존 gRPC 경로만 탄다. 빈 문자열로 넘기면 기본값도 끈다.
const GRAPHQL_DEFAULTS = {
  'sui-testnet': 'https://graphql.testnet.sui.io/graphql',
  'sui-mainnet': 'https://graphql.mainnet.sui.io/graphql',
}
export const GRAPHQL_URL =
  process.env.HUMMING_GRAPHQL_URL !== undefined
    ? process.env.HUMMING_GRAPHQL_URL || null
    : (GRAPHQL_DEFAULTS[NETWORK] ?? null)
// 이보다 작은 갭은 gRPC로 그대로 따라잡는다 (배포 재시작 수준은 수십 개라 백필이 필요 없다)
export const GAP_BACKFILL_MIN = Number(process.env.HUMMING_GAP_BACKFILL_MIN ?? 300)
