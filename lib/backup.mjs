...
export const STATE_FILES = [
  NETWORK === 'localnet' ? 'wallet-keys.json' : `wallet-keys.${NETWORK}.json`,
  ...
  // 모더레이션 증거: 신고 원장·운영자 숨김 목록 (분쟁·이의제기 재구성용)
  `reports.${NETWORK}.jsonl`,
  `hidden-posts.${NETWORK}.json`,
]
...
// 우아한 종료용: 디바운스 대기 중인 변경이 있으면 지금 스냅샷을 찍는다.
// 마지막 가입·계정 변경이 종료와 함께 백업에서 유실되는 창을 없앤다.
