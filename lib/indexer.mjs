...
// dedupe가 실제로 필요한 창은 "자기 tx 동기 주입 ~ 폴링 커서가 그 이벤트를 지나갈 때까지"(수 초)뿐.
// 커서가 지나간 이벤트는 다시 오지 않으므로, 상한 초과 시 오래된 키를 잘라도 안전하다.
...
export const state = {
  ...
  // Subscribed/TipSent/PostPurchased 원장 — 백필이 타입별 병렬이라 타입 안에서만
  // 시간 오름차순이고 전체로는 뒤섞임. 소비자는 반드시 timestampMs로 정렬할 것
  earnings: [],
}
