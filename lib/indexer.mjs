...
import { PKG, FEED, GAP_BACKFILL_MIN } from './config.mjs'
import {
  checkpointToEvents, getCheckpointEvents, serviceInfo, subscribeCheckpoints, isUnavailable, isRateLimited,
} from './grpc.mjs'
import { fetchGapEvents, shouldBackfill } from './graphql.mjs'
...
export const state = {
  ...
  earnings: [],
  // 플랫폼 수수료(bps). 부팅 시 FeeConfig 객체에서 읽고 FeeChanged로 갱신 — 앱 카피와
  // 견적 표시의 원본. null이면 아직 모름(부팅 전)
  feeBps: null,
}
...
function apply(shortType, ev, fallbackTs) {
  ...
  switch (shortType) {
    ...
    case 'creator_prefs::PrefsChanged':
      ...
      break
    case 'platform::FeeChanged':
      state.feeBps = Number(p.fee_bps)
      break
  }
  if (
...
function mergeChainState(cs) {
  ...
  // 객체의 현재값이 곧 최신 — 저널 리플레이의 FeeChanged보다 우선한다
  if (cs.feeBps != null) state.feeBps = cs.feeBps
...
// 요청 제한에 걸렸을 때의 재시도 간격 — 같은 배치를 다시 읽는다 (건너뛰지 않는다)
const RATE_LIMIT_BACKOFF_MS = [2000, 5000, 10000, 30000]
const sleep = ms =>
  new Promise(r => {
...
// 큰 갭의 GraphQL 백필: 구간의 우리 이벤트를 이벤트 필터 쿼리로 통째로 받아 반영하고
// 커서를 target으로 점프한다. 이벤트마다 자기 타임스탬프를 갖고 있어 fallbackTs가 필요 없고,
// dedupe 키가 저널·스트림과 같은 형태라 이미 관측한 이벤트는 걸러진다. 프루닝 하한 밑의
// 구간도 GraphQL이 보관하는 한 복원되므로 pruneRecover(이력 포기)보다 먼저 시도한다.
async function backfillGap(from, through) {
  const events = await fetchGapEvents(from - 1, through, EVENT_TYPES.map(t => `${PKG}::${t}`))
  ingestEvents(events)
  console.log(
    `   ⏩ 갭 백필: 체크포인트 ${from}..${through} (${through - from + 1}개)을 GraphQL 이벤트 ${events.length}건으로 복원`,
  )
  ckpt = through
  lastCheckpointAt = Date.now()
  maybeSaveCursor(true)
...
async function catchUp(target = null) {
  ...
  if (shouldBackfill(latest - ckpt, { min: GAP_BACKFILL_MIN })) {
    try {
      return await backfillGap(ckpt + 1, latest)
    } catch (e) {
      console.warn(`   ⚠️ GraphQL 갭 백필 실패 (${e.message}) — gRPC 체크포인트 읽기로 진행`)
    }
  }
  ...
  while (ckpt < latest) {
    ...
    try {
      ...
      if (isUnavailable(e)) return pruneRecover(latest)
      // 공공 풀노드 요청 제한: 죽지 않고 같은 배치를 잠시 뒤 다시 읽는다
      if (isRateLimited(e)) {
        const wait = RATE_LIMIT_BACKOFF_MS[Math.min(limited++, RATE_LIMIT_BACKOFF_MS.length - 1)]
        console.warn(`   ⏳ 요청 제한 (체크포인트 ${seqs[0]}..): ${wait}ms 후 재시도`)
        await sleep(wait)
        continue
      }
      throw e
