...
function loadSecret() {
  ...
  writeFileAtomic(SECRET_FILE, secret.toString('base64') + '\n', { mode: 0o600 })
  return secret
}
...
export function makeJwt(did, scope, ttlS, jti = undefined) {
  const now = Math.floor(Date.now() / 1000)
  const body = [
    b64u({ typ: 'JWT', alg: 'HS256' }),
    b64u({ scope, sub: did, aud: 'did:web:localhost', iat: now, exp: now + ttlS, ...(jti && { jti }) }),
  ].join('.')
  return `${body}.${sign(body)}`
}

// access/refresh 쌍 발급: createAccount/createSession/refreshSession 공용.
// 쌍이 공유하는 세션 ID(jti)로 deleteSession이 두 토큰을 한 번에 폐기할 수 있다.
export const sessionTokens = did => {
  const jti = crypto.randomBytes(12).toString('base64url')
  return {
    accessJwt: makeJwt(did, 'com.atproto.access', ACCESS_TTL_S, jti),
    refreshJwt: makeJwt(did, 'com.atproto.refresh', REFRESH_TTL_S, jti),
  }
}
