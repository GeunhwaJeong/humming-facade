...
const CURSOR_TMP_URL = new URL(`../indexer-cursor.${NETWORK}.json.tmp`, import.meta.url)
...
export function loadJournal() {
  ...
  for (const line of raw.split('\n')) {
    ...
    try {
      ...
      // 크래시로 잘린 마지막 줄 — 커서가 그 앞에 저장돼 있어 폴링이 다시 채운다
    }
  }
  return events
}
...
export function saveCursor(checkpoint) {
  fs.writeFileSync(CURSOR_TMP_URL, JSON.stringify({ checkpoint }), { mode: 0o600 })
  fs.renameSync(CURSOR_TMP_URL, CURSOR_URL)
}
