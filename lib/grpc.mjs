...
// 공공 풀노드의 요청 제한(RESOURCE_EXHAUSTED/429) — 복구 불가가 아니라 "천천히 다시"의 신호
export const isRateLimited = e =>
  e?.code === 'RESOURCE_EXHAUSTED' || /rate.?limit|too many requests|\b429\b|exhausted/i.test(String(e?.message))
...
// ---- 객체/상태 읽기 ----
// gRPC json 렌더링은 JSON-RPC content.fields와 달리 래퍼({type, fields})가 없는
// 평면 구조다: Table → {id: "0x..", size}, UID → 문자열, 중첩 struct → 평면 객체.
// 값 규약(u64→문자열, 주소 0x-패딩, Option→null)은 parsedJson과 동일 (실측 확인).

// 단건 조회 — 객체가 없으면 null (denylist처럼 부재가 정보인 조회용)
export async function getObjectJson(objectId) {
  const {
    objects: [o],
  } = await grpcClient.getObjects({ objectIds: [objectId], include: { json: true } })
  return o instanceof Error ? null : o.json
}

// 일괄 조회 — 입력 순서 유지, 실패 항목은 null
export async function getObjectsJson(ids) {
  if (!ids.length) return []
  const { objects } = await grpcClient.getObjects({ objectIds: ids, include: { json: true } })
  return objects.map(o => (o instanceof Error ? null : o.json))
}

export async function listAllDynamicFields(parentId) {
  const out = []
  let cursor = null
  do {
    const page = await grpcClient.listDynamicFields({ parentId, cursor })
    out.push(...page.dynamicFields)
    cursor = page.hasNextPage ? page.cursor : null
  } while (cursor)
  return out
}

export async function listOwnedJson(owner, type) {
  const out = []
  let cursor = null
  do {
    const page = await grpcClient.listOwnedObjects({ owner, type, cursor, include: { json: true } })
...
// JSON-RPC getChainIdentifier와 동일한 8-hex 식별자 — gRPC는 제네시스 다이제스트
// 전체를 base58로 주므로 앞 4바이트만 hex로 변환한다
export async function chainIdentifier() {
  return Buffer.from(fromBase58((await serviceInfo()).chainId)).subarray(0, 4).toString('hex')
}
